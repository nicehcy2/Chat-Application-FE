...
- API / WebSocket 주소가 코드에 하드코딩되어 있으므로 `.env`로 분리
- 공통 API 클라이언트 추출 (반복되는 fetch 로직 통합)
- 페이지/컴포넌트 한글 문자열 인코딩 점검
- 미완성 라우트(`Home`, `ChatRoomExplore`, `ChatCreate`) 완성 및 연결
- 에러 처리 및 사용자 피드백 강화 (네트워크 오류, 인증 실패 등)
- 테스트 코드 보강
- 배포 환경 분리 (개발 / 운영 서버 주소 구분)
